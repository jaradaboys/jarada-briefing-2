...
function buildObservationInsightLine(form, visionResult) {
  ...
  if (hasAnyText(text, ["스케치", "아이디어"]) && hasAnyText(text, ["재밌", "즐겁", "좋아", "흥미"])) {
    return `이번 달 ${childName}는 ${topic}을 하며, 머릿속에 떠오른 장면을 자기 방식으로 화면에 옮겨보았습니다. 아이디어를 떠올리고 스케치하는 과정 자체를 즐겁게 이어간 점이 인상적이었습니다.`;
  }

  if (hasAnyText(text, ["스케치", "아이디어"])) {
    return `이번 달 ${childName}는 ${topic}을 하며, 자신이 떠올린 생각을 화면에 옮겨보는 시간을 가졌습니다.`;
  }
...
function buildMonthlyGrowthInterpretation(form, visionResult) {
  ...
  if (task?.key === "toolMastery") {
    return `이번 달에는 ${childName}가 도구와 재료를 다루며 손끝의 시도를 쌓아가는 과정이 중요했습니다. 익숙하지 않은 방식도 경험해보며 “내가 할 수 있다”는 감각을 넓혀가는 시간이었습니다.`;
  }

  if (task?.key === "innerSelfRegulation") {
    return `이번 달에는 ${childName}가 자신의 마음과 생각을 수업 안에서 조금씩 표현해보는 경험이 중요했습니다. 말과 작품으로 마음을 꺼내보는 과정이 정서적 안정감으로 이어질 수 있습니다.`;
  }

  if (task?.key === "metacognitiveSelfRegulation" || task?.key === "selfUnderstanding") {
    return `이번 달에는 ${childName}가 자신의 생각과 작업 방식을 조금 더 의식하며, 어떤 방향으로 이어갈지 스스로 살펴보는 과정이 중요했습니다.`;
  }

  if (task?.key === "emotionalSelfManagement") {
...
function buildContextualNextPlan(form, visionResult) {
  ...
  return `다음 달에는 ${childName}가 이번 달에 보여준 흥미와 참여를 바탕으로, 자신의 생각을 더 분명하게 표현하고 끝까지 이어가보는 경험을 확장하겠습니다.`;
}
...
function buildContextualExpectation(form) {
  ...
  if (task?.key === "innerSelfRegulation" || task?.key === "emotionalSelfManagement") {
    return `이런 경험이 반복될수록 ${childName}는 자신의 마음을 알아차리고, 어려운 순간에도 다시 성장의 방향으로 돌아오는 힘을 키워갈 수 있습니다.`;
  }
...
- "발달 흐름", "누적 기본값", "중심 성장축", "단순히 ~보다" 같은 표현은 학부모용 문장에 쓰지 않는다.
- 선택된 키워드를 나열하지 말고, 이번 달 실제 수업 장면으로 번역해 쓴다.
- 가정 연계 포인트에는 부모님이 바로 사용할 수 있는 질문 예시를 1~2개 넣는다.
- 학부모가 읽었을 때 "우리 아이가 지금 어떤 시기이구나"라고 느끼도록 쉽게 쓴다.

[성장 인사이트 제목 원칙]
- 첫 줄에는 이번 달 수업의 의미를 한 문장으로 압축한 성장 인사이트 제목을 쓴다.
- 제목은 학부모님이 브리핑을 읽고 싶어지게 만드는 첫인상 문장이다.
- 활동명만 쓰지 말고, 아이의 장면과 성장 의미가 함께 느껴지게 쓴다.
...
<section style={{ ...styles.card, marginTop: 24 }}>
  ...
  <div style={{ ...styles.softBox, marginTop: 18 }}>
    <h3>심화브리핑 제목</h3>
    <div style={styles.sectionHint}>
      학부모님이 브리핑을 읽기 시작하게 만드는 첫 문장입니다. 아이의 수업 장면과 성장 의미가 함께 느껴지도록 선택하거나 직접 수정합니다.
    </div>

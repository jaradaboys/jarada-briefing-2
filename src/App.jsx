    ...
    tasks: [
      {
        ...
        axis: "자아효능감",
        training: "인지적 자기조절",
        parentText: "하고 싶은 행동을 바로 하기보다 잠깐 멈추고 기다려보는 힘",
      ...
      },
      {
        ...
        axis: "사회적효능감",
        training: "정서적 자기조절",
        parentText: "자신의 마음을 알아차리고 말로 표현해보는 힘",
      ...
      },
      {
        ...
        axis: "사회적효능감",
        training: "정서적 자기조절",
        parentText: "부모와 떨어져 교사와 교실을 안전하게 느끼는 힘",
    ...
    tasks: [
      {
        ...
        axis: "자아효능감",
        training: "인지적 자기조절",
        parentText: "스스로 계획하고 어려워도 끝까지 해보는 힘",
      ...
      },
      {
        ...
        axis: "사회적효능감",
        training: "정서적 자기조절",
        parentText: "친구와 생각을 맞추고 관계 안에서 인정받아보는 힘",
      ...
      },
      {
        ...
        axis: "자아효능감",
        training: "인지적 자기조절",
        parentText: "도구를 안전하게 다루며 내가 해낼 수 있다는 감각을 쌓는 힘",
      ...
      },
      {
        ...
        axis: "자아효능감",
        training: "인지적 자기조절",
        parentText: "맡은 일을 스스로 챙기고 책임지는 힘",
    ...
    tasks: [
      {
        ...
        axis: "자아효능감",
        training: "인지적 자기조절",
        parentText: "자신의 상태를 알고 자기에게 맞는 방향을 찾아가는 힘",
      ...
      },
      {
        ...
        axis: "사회적효능감",
        training: "정서적 자기조절",
        parentText: "어려운 마음을 피하지 않고 성장의 힘으로 바꾸는 경험",
      ...
      },
      {
        ...
        axis: "자아효능감",
        training: "인지적 자기조절",
        parentText: "내가 잘하는 것과 흔들리는 지점을 알아차리는 힘",
      ...
      },
      {
        ...
        axis: "자아효능감",
        training: "인지적 자기조절",
        parentText: "익숙한 수준을 넘어 더 어려운 기준에 도전해보는 힘",
      ...
      },
      {
        ...
        axis: "자아효능감",
        training: "인지적 자기조절",
        parentText: "시켜서가 아니라 스스로 의미를 찾고 해내는 힘",
      ...
      },
      {
        ...
        axis: "사회적효능감",
        training: "정서적 자기조절",
        parentText: "팀 안에서 자신의 역할과 기여 방식을 찾아가는 힘",
...
function getGrowthTasksForAge(ageBand) {
  return ageGrowthTasks[ageBand]?.tasks || [];
}

function getSelectedGrowthTask(form) {
  const tasks = getGrowthTasksForAge(form.ageBand);
  return tasks.find((task) => task.key === form.growthTask) || tasks[0] || null;
}

function buildGrowthTaskSummary(form) {
  const task = getSelectedGrowthTask(form);
  const keywords = form.growthTaskKeywords || [];

  if (!task) {
    return "아직 이번달 성장과제가 선택되지 않았습니다.";
  }

  const keywordText = keywords.length ? keywords.join(" · ") : "세부 키워드 미선택";
...
function buildGrowthTaskParentLine(form) {
...
function getBriefingChildName(studentName) {
  const name = String(studentName || "").trim().replace(/\s+/g, "");
  if (!name) return "아이";
  if (name.length === 1) return name;

  const givenName = name.length >= 3 ? name.slice(1) : name.slice(1);
  if (!givenName) return name;
  if (givenName.endsWith("이")) return givenName;

  const lastChar = givenName[givenName.length - 1];
  return hasKoreanFinalConsonant(lastChar) ? `${givenName}이` : givenName;
}

function cleanBriefingText(text) {
  return String(text || "")
    .replace(/\s+/g, " ")
    .replace(/작업중/g, "작업을 하며")
    .replace(/하는 중/g, "이어가는 중")
    .replace(/\s*\.\s*/g, ". ")
    .trim();
}

function extractActivityTopic(form, visionResult) {
  const text = cleanBriefingText(
    `${form.artworkFlow || ""} ${form.memo || ""} ${visionResult?.flow_summary || ""}`
  );

  if (!text) return form.project || "작업";

  const drawingMatch = text.match(/([가-힣A-Za-z0-9]+)\s*(그리기|만들기|스케치|작업|표현)/);
  if (drawingMatch) {
    const topic = drawingMatch[1];
    const action = drawingMatch[2];
    if (topic && action && !["아이디어", "작품", "수업"].includes(topic)) {
      return `${topic} ${action}`;
    }
  }

  const favoriteWords = ["도라에몽", "포켓몬", "마인크래프트", "로블록스", "캐릭터", "공룡", "자동차", "로봇"];
  const foundFavorite = favoriteWords.find((word) => text.includes(word));
  if (foundFavorite) {
    return foundFavorite === "캐릭터" ? "좋아하는 캐릭터" : `${foundFavorite}를 주제로 한 작업`;
  }

  if (text.includes("스케치")) return "아이디어 스케치";
  if (text.includes("연작")) return "연작";
  if (text.includes("협동")) return "협동작업";
  if (text.includes("100호")) return "100호캔버스";
  return form.project || "작업";
}
...
function buildBriefingTitleCandidates(form, visionResult) {
  ...
  if (hasAnyText(text, ["재밌", "즐겁", "흥미", "좋아"])) {
    candidates.push("재미가 몰입이 되고, 몰입이 표현으로 이어진 수업");
  }

  if (hasAnyText(text, ["끝까지", "완성", "이어", "지속", "몰입"])) {
  ...
  } else if (task?.key === "behavioralSelfRegulation") {
    candidates.push("기다림 속에서 자기조절이 자라난 시간");
  } else if (task?.key === "innerSelfRegulation") {
    candidates.push("마음을 알아차리고 표현해본 시간");
  } else if (task?.key === "metacognitiveSelfRegulation") {
  ...
  candidates.push("표현의 즐거움이 다음 시도로 이어진 시간");
  candidates.push("자기 속도로 한 걸음 자란 시간");
...
function getSelectedBriefingTitle(form, visionResult) {
  ...
  return buildBriefingTitleCandidates(form, visionResult)[0] || "이번 달 작은 성장이 쌓인 시간";
}

function buildAgeGrowthContextLine(form) {
  const childName = getBriefingChildName(form.student);
  const task = getSelectedGrowthTask(form);
  const age = form.studentMeta?.age;
  const agePrefix = age ? `${age}세인 ${childName}는` : `${childName}는`;

  if (task?.parentText) {
    return `${agePrefix} 지금 ${task.parentText}이 중요해지는 시기에 있습니다.`;
  }

  if (form.ageBand === "6–7세") {
    return `${agePrefix} 하고 싶은 행동을 잠깐 멈추고, 자신의 마음을 말로 표현해보는 경험이 중요해지는 시기에 있습니다.`;
  }

  if (form.ageBand === "8–11세") {
    return `${agePrefix} 스스로 정하고 끝까지 해보는 힘과 친구와 생각을 맞춰가는 경험이 중요해지는 시기에 있습니다.`;
  }

  if (form.ageBand === "12–13세") {
    return `${agePrefix} 자신의 상태를 이해하고 스스로 방향을 찾아가는 경험이 중요해지는 시기에 있습니다.`;
  }
...
function buildObservationInsightLine(form, visionResult) {
  ...
  if (hasAnyText(text, ["스케치", "아이디어"]) && hasAnyText(text, ["재밌", "즐겁", "좋아", "흥미"])) {
    return `이번 달 ${childName}는 ${topic}에서 아이디어를 떠올리고 스케치하는 과정을 즐겁게 이어갔습니다.`;
  }

  if (hasAnyText(text, ["스케치", "아이디어"])) {
    return `이번 달 ${childName}는 ${topic}에서 자신이 떠올린 생각을 화면에 옮겨보는 시간을 가졌습니다.`;
  }

  if (hasAnyText(text, ["친구", "함께", "협동"])) {
    return `이번 달 ${childName}는 ${topic} 안에서 친구와 함께하며 자신의 생각을 꺼내고 맞춰보는 경험을 했습니다.`;
  }

  if (hasAnyText(text, ["끝까지", "완성", "몰입", "지속"])) {
    return `이번 달 ${childName}는 ${topic}을 붙잡고 끝까지 이어가려는 모습을 보여주었습니다.`;
  }
...
function buildGrowthHistoryLine(form) {
  ...
  const main = profile.dominantMainKeywords || [];
  const sub = profile.dominantSubKeywords || [];

  if (!profile.sourceCount) {
    return "";
  }

  const combined = [...main, ...sub].join(" ");

  if (hasAnyText(combined, ["관계조율", "친구", "함께", "의견조율", "갈등해결"])) {
    return `지난 기록에서도 ${childName}는 친구와 함께하는 장면에서 자신의 생각을 표현하고, 관계 안에서 맞춰가려는 흐름이 꾸준히 보였습니다.`;
  }

  if (hasAnyText(combined, ["자기주도성", "선택하기", "스스로시작", "과제지속", "완수경험"])) {
    return `지난 기록에서도 ${childName}는 스스로 선택하고 이어가려는 모습이 조금씩 쌓여왔습니다.`;
  }

  if (hasAnyText(combined, ["감정표현", "감정조절", "화", "감정말하기"])) {
    return `지난 기록에서도 ${childName}는 감정이 올라오는 순간을 지나 다시 수업 흐름으로 돌아오려는 경험을 쌓아왔습니다.`;
  }
...
function buildMonthlyGrowthInterpretation(form, visionResult) {
  ...
  if (task?.key === "cognitiveSelfRegulation") {
    return `이번 달 수업에서는 ${childName}가 ${topic}에 흥미를 느끼고, 그 안에서 무엇을 어떻게 표현할지 스스로 정해보는 과정이 중요했습니다. 처음 떠올린 생각을 스케치로 옮겨보는 경험은 스스로 계획하고 끝까지 해보는 힘으로 이어질 수 있습니다.`;
  }

  if (task?.key === "socialSelfRegulation") {
    return `이번 달 수업에서는 ${childName}가 자기 생각을 꺼내는 동시에 친구와의 흐름도 함께 살피는 경험이 중요했습니다. 관계 안에서 맞춰보는 작은 장면들이 사회성코칭의 중요한 출발점이 됩니다.`;
  }

  if (task?.key === "toolMastery") {
    return `이번 달 수업에서는 ${childName}가 도구와 재료를 다루며 손끝의 시도를 쌓아가는 과정이 중요했습니다. 익숙하지 않은 방식도 경험해보며 “내가 할 수 있다”는 감각을 넓혀가는 시간이었습니다.`;
  }

  if (task?.key === "innerSelfRegulation") {
    return `이번 달 수업에서는 ${childName}가 자신의 마음과 생각을 수업 안에서 조금씩 표현해보는 경험이 중요했습니다. 말과 작품으로 마음을 꺼내보는 과정이 정서적 안정감으로 이어질 수 있습니다.`;
  }

  if (hasAnyText(text, ["스케치", "아이디어", "재밌", "즐겁"])) {
    return `이번 달 수업에서는 ${childName}가 좋아하는 주제 안에서 생각을 떠올리고, 그 생각을 표현으로 옮겨보는 과정이 중요했습니다.`;
  }

  return `이번 달 수업에서는 ${childName}가 자신의 방식으로 참여하고, 그 과정에서 새롭게 보인 변화와 다음 성장을 함께 확인할 수 있었습니다.`;
}
...
function buildContextualHomeGuide(form, visionResult) {
  ...
  if (hasAnyText(text, ["스케치", "아이디어", "그리기", "도라에몽", "캐릭터"])) {
    return `가정에서는 완성된 그림을 평가하기보다 “이 장면은 어떻게 생각해서 그린 거야?”처럼 아이디어를 물어봐 주시면 좋습니다. ${childName}가 떠올린 생각을 말로 정리해보는 경험이 다음 작업의 자신감으로 이어질 수 있습니다.`;
  }

  if (task?.key === "socialSelfRegulation" || hasAnyText(text, ["친구", "함께", "의견", "갈등"])) {
    return `가정에서는 친구 이야기가 나올 때 바로 판단하기보다 “그때 너는 어떤 생각이었어?”, “친구는 어떤 마음이었을까?”처럼 서로의 입장을 떠올려보는 질문이 도움이 됩니다.`;
  }

  if (task?.key === "cognitiveSelfRegulation" || hasAnyText(text, ["끝까지", "계획", "완성", "몰입"])) {
    return `가정에서는 결과보다 과정을 짚어주시면 좋습니다. “처음 생각한 걸 끝까지 이어갔네”, “어려운 부분을 다시 해봤네”처럼 구체적으로 알아봐 주면 스스로 해냈다는 감각이 더 잘 남습니다.`;
  }

  return buildHomeGuide(form);
}

function buildContextualNextPlan(form, visionResult) {
  const childName = getBriefingChildName(form.student);
  const task = getSelectedGrowthTask(form);
  const topic = extractActivityTopic(form, visionResult);
  ...
  if (task?.key === "toolMastery") {
    return `다음 달에는 ${childName}가 도구를 다루는 자신감을 바탕으로 더 안정적으로 시도하고, 완성까지 이어가는 경험을 넓혀가겠습니다.`;
  }

  return `다음 달에는 ${childName}가 이번 달에 보인 흥미와 참여 흐름을 바탕으로, 자신의 생각을 더 분명하게 표현하고 끝까지 이어가보는 경험을 확장하겠습니다.`;
}
...
function buildContextualExpectation(form) {
  ...
  if (task?.key === "cognitiveSelfRegulation") {
    return `이런 경험이 반복되면 ${childName}는 좋아하는 마음에서 출발해 스스로 계획하고 완성해보는 힘을 조금 더 안정적으로 쌓아갈 수 있습니다.`;
  }

  if (task?.key === "socialSelfRegulation") {
    return `이런 경험이 반복되면 ${childName}는 관계 안에서 자기 생각을 표현하면서도 친구와 함께 맞춰가는 힘을 조금 더 편안하게 사용할 수 있습니다.`;
  }

  if (task?.key === "innerSelfRegulation" || task?.key === "emotionalSelfManagement") {
    return `이런 경험이 반복되면 ${childName}는 자신의 마음을 알아차리고, 어려운 순간에도 다시 성장의 방향으로 돌아오는 힘을 키워갈 수 있습니다.`;
  }

  return `이런 경험이 반복되면 ${childName}는 자신만의 속도로 시도하고, 그 안에서 “나는 할 수 있다”는 믿음을 조금씩 쌓아갈 수 있습니다.`;
}
...
function generatePreview(form, visionResult) {
  ...
  const childName = getBriefingChildName(form.student);
  const title = getSelectedBriefingTitle(form, visionResult);
  const observationLine = buildObservationInsightLine(form, visionResult);
  const ageContextLine = buildAgeGrowthContextLine(form);
  const monthlyInterpretation = buildMonthlyGrowthInterpretation(form, visionResult);
  const historyLine = buildGrowthHistoryLine(form);
  const homeLine = buildContextualHomeGuide(form, visionResult);
  const nextPlanLine = buildContextualNextPlan(form, visionResult);
  const expectationLine = buildContextualExpectation(form);
  ...
  return [
    ...
    "안녕하세요.",
    "",
    `${childName}의 이번 달 심화브리핑입니다.`,
    "",
    observationLine,
    "",
    `${ageContextLine} ${monthlyInterpretation}`,
    "",
    historyLine,
    "",
    homeLine,
...
function generatePrompt(form, visionResult) {
  ...
  const selectedGrowthTask = getSelectedGrowthTask(form);

  const selectedGrowthKeywords = [
    selectedGrowthTask?.label,
    ...(form.growthTaskKeywords || []),
    form.ageDomain,
    ...(form.ageSubKeywords || []),
    form.project,
  ...
  const briefingTitleCandidates = buildBriefingTitleCandidates(form, visionResult);
  const selectedBriefingTitle = getSelectedBriefingTitle(form, visionResult);

  return `너는 자라다교육의 심화브리핑 작성 보조자다.
교사의 관찰 메모를 학부모에게 전달할 월간 심화 브리핑으로 정리한다.

[자라다 성장 구조]
자라다는 아이의 성장을 건강한 자립과 자존감의 관점에서 본다.
자존감은 반복된 자아실현 경험을 통해 자라며,
그 경험은 자아효능감과 사회적효능감으로 나뉜다.

자아효능감은 아이가 스스로 선택하고, 계획하고, 끝까지 해내며, 이전의 한계를 넘어보는 경험에서 자란다.
사회적효능감은 아이가 관계 안에서 인정받고, 친구와 조율하고, 함께 성취하는 경험에서 자란다.

이 성장을 위해 수업 안에서는 인지적 자기조절과 정서적 자기조절이 함께 훈련된다.
또한 아이가 자아실현을 반복하려면 소속감, 효능감, 안전감이 있는 수업 환경이 필요하다.

[작성 원칙]
- 브리핑은 활동 설명이 아니라 아이의 성장 흐름을 정리하는 글이다.
- 아이를 결과로 판단하지 말고, 성장하는 과정으로 해석한다.
- 교사 메모를 그대로 복사하지 말고, JARVIS 누적 기록과 이번달 관찰 장면을 연결한다.
- 학부모 니즈는 부모가 기대하는 방향으로 반영하되, 아이를 평가하거나 단정하지 않는다.
- 작품 결과보다 아이가 선택하고, 버티고, 조율하고, 다시 시도한 경험을 중심으로 쓴다.
- "좋았다", "잘했다"보다 어떤 성장 경험이 있었는지 설명한다.
- 문장은 따뜻하지만 전문적으로 작성한다.
- 전체 문장은 6~8문장 정도로 정리한다.
- "아이"라는 표현보다 가능한 한 학생 이름을 사용한다.

[학부모용 문장 원칙]
- 교사용 공식 키워드는 학부모가 이해하기 쉬운 말로 풀어쓴다.
- "인지적 자기조절"은 "스스로 계획하고 끝까지 해보는 힘"으로 풀어쓴다.
- "사회적 자기조절"은 "친구와 생각을 맞춰가는 힘"으로 풀어쓴다.
- "메타인지적 자기조절"은 "자신의 상태를 알고 방향을 찾아가는 힘"으로 풀어쓴다.
- "정서적 자기경영"은 "어려운 마음을 피하지 않고 성장의 힘으로 바꾸는 경험"으로 풀어쓴다.
- 학부모가 읽었을 때 "우리 아이가 지금 어떤 시기이구나"라고 느끼도록 쉽게 쓴다.

[성장 인사이트 제목 원칙]
- 첫 줄에는 이번 달 수업의 의미를 한 문장으로 압축한 성장 인사이트 제목을 쓴다.
- 제목은 활동명만 쓰지 말고, 아이의 장면과 성장 의미가 함께 느껴지게 쓴다.
...
[이번달 성장 인사이트 제목]
선택된 제목: ${selectedBriefingTitle}
추천 제목 후보:
${briefingTitleCandidates.map((title, index) => `${index + 1}. ${title}`).join("\n")}
...
[이번달 수업 선택값]
이번달 성장과제: ${selectedGrowthTask?.label || "미선택"}
성장과제 의미: ${selectedGrowthTask?.description || "미입력"}
학부모용 해석: ${selectedGrowthTask?.parentText || "미입력"}
성장과제 세부 키워드: ${(form.growthTaskKeywords || []).join(", ") || "미선택"}
이번달 성장 키워드 전체: ${selectedGrowthKeywords.join(", ") || "미선택"}
...
[최종 출력]
첫 줄에는 성장 인사이트 제목을 쓴다.
그 다음부터 학부모에게 바로 전달 가능한 브리핑 문장만 작성한다.
대괄호 항목명은 쓰지 말고 자연스러운 문단으로 작성한다.
"단순히 ~보다", "누적 기본값", "발달 흐름", "인지적 자기조절" 같은 표현은 학부모용 문장에 그대로 쓰지 않는다.`;
}
...
const [showPrompt, setShowPrompt] = useState(false);
  ...
  const briefingTitleCandidates = useMemo(
    () => buildBriefingTitleCandidates(form, visionResult),
    [form, visionResult]
  );
...
// TODO: 실제 Google Form API 연계 후에는 응답 결과가 0건일 때 이 분기로 대체합니다.
 if (studentName.includes(GOOGLE_FORM_TEST_KEYWORDS.empty)) {
  setGoogleFormCandidates([]);
...
const isCurrentGoogleFormCandidate = (candidate) => {
  ...
  const saveStudentProfilePatch = (name, patch = {}) => {
    ...
    setStudentProfiles((prev) => {
      ...
      return {
        ...
        [studentName]: {
          ...existing,
          student: studentName,
          parentNeeds: patch.parentNeeds ?? existing.parentNeeds ?? emptyNeeds(),
          jarvisObservations: patch.jarvisObservations ?? existing.jarvisObservations ?? [],
          growthProfile: patch.growthProfile ?? existing.growthProfile ?? emptyGrowthProfile(),
          growthBase: Object.prototype.hasOwnProperty.call(patch, "growthBase")
            ? patch.growthBase
            : existing.growthBase ?? null,
          studentMeta: Object.prototype.hasOwnProperty.call(patch, "studentMeta")
            ? patch.studentMeta
            : existing.studentMeta ?? null,
          googleFormSource: Object.prototype.hasOwnProperty.call(patch, "googleFormSource")
            ? patch.googleFormSource
            : existing.googleFormSource,
          ...patch,
...
if (profile?.googleFormSource === null) {
  ...
  const importManualJarvis = () => {
    ...
    const imported = parseManualJarvisText(manualJarvisText, name, form.ageBand);
    ...
    if (form.studentMeta) {
      profilePatch.studentMeta = form.studentMeta;
    }
    ...
    saveStudentProfilePatch(name, profilePatch);
    setManualJarvisText("");
    alert(`${imported.length}개의 관찰일지를 추가했고, 학생 기본값에도 저장했습니다.`);
  };
...
setForm((prev) => ({
  ...
  studentMeta: studentMeta || prev.studentMeta,
  jarvisObservations: merged,
  growthProfile,
  growthBase,
}));
...
if (studentMeta) {
  profilePatch.studentMeta = studentMeta;
}

saveStudentProfilePatch(studentNameForProfile, profilePatch);
alert(`${incoming.length}개의 JARVIS 관찰일지를 불러왔고, 학생 기본값에도 저장했습니다.`);
  ...
  const removeObservation = (id) => {
    ...
    const growthBase = buildStudentGrowthBase(next);

    setForm((prev) => ({
      ...prev,
      jarvisObservations: next,
      growthProfile,
      growthBase,
    }));
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        <div style={styles.topGrid}>
          <div style={{ ...styles.card, textAlign: "center", minHeight: 150, padding: 24, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
            <h1 style={styles.title}>JARADA 성장기반 심화브리핑</h1>
            <p style={{ ...styles.subtitle, maxWidth: 860 }}>
              매달의 수업 기록을 아이의 성장 서사로 연결합니다.
              <br />
              작품, 학부모 니즈, JARVIS 기록, 교사 해석을 바탕으로 아이의 다음 성장을 정리합니다.
            </p>
          </div>

          <div style={styles.darkCard}>
            <div style={{ fontWeight: 900, marginBottom: 16, fontSize: 24 }}>교사의 사명</div>

            <div style={{ lineHeight: 1.9, fontSize: 17, maxWidth: 560 }}>
              우리는 아이를 결과로 판단하지 않고, 성장하는 과정으로 바라봅니다.
              <br />
              작은 선택, 끝까지 해낸 경험, 관계 안에서 인정받은 순간을 발견하고
              <br />
              아이 안에 “나는 할 수 있다”는 믿음이 자라도록 돕습니다.
            </div>
          </div>
        </div>
        ...
        <div style={styles.grid}>
          ...
          {/* Google Form integration panel */}
            ...
            <h2 style={styles.sectionTitle}>1. 학생 / 학부모 심화설문 연계</h2>
            <div style={styles.sectionHint}>
  학부모 심화설문 결과는 학생별 성장 니즈로 저장되며, 구글폼 응답과 JARVIS 기록을 연결해 심화브리핑의 기본값으로 활용합니다.
</div>
 ...
 <div style={styles.sectionHint}>
  ...
  구글폼 연계 상태
  <br />
 <strong>구글폼 API 연계 전 단계 · 현재는 임시 후보 데이터로 연결 흐름을 검증 중입니다.</strong>
{(() => {
  const source = getCurrentGoogleFormSource();
  ...
  return (
    <>
      <div style={{ fontSize: 13, color: "#475569", marginTop: 8 }}>
        연결된 구글폼 응답: {source.submittedAt} / 연락처 뒷자리{" "}
        {source.phoneLast4}
      </div>
    ...
    <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
      {googleFormCandidates.map((candidate) => (
        <div key={candidate.id} style={styles.recordCard}>
          ...
          {isCurrentGoogleFormCandidate(candidate) && (
...
<div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
  <button
    type="button"
    disabled={isCurrentGoogleFormCandidate(candidate)}
    style={{
      ...styles.secondaryBtn,
      opacity: isCurrentGoogleFormCandidate(candidate) ? 0.5 : 1,
      cursor: isCurrentGoogleFormCandidate(candidate) ? "not-allowed" : "pointer",
    }}
    onClick={(event) => {
      event.preventDefault();
      event.stopPropagation();

      if (isCurrentGoogleFormCandidate(candidate)) return;

      connectGoogleFormCandidate(candidate);
    }}
  >
    {isCurrentGoogleFormCandidate(candidate) ? "이미 연결됨" : "이 설문 연결하기"}
  </button>
</div>
        </div>
      ))}
...
<section style={{ ...styles.card, marginTop: 24 }}>
  ...
  <div style={styles.softBox}>
    <h3>이번달 성장과제</h3>
    <div style={styles.sectionHint}>
      연령 흐름에 따라 이번 달 아이에게 필요한 핵심 성장 방향을 선택합니다.
    </div>

    <div style={{ ...styles.miniStat, marginTop: 12 }}>
      <strong>{ageGrowthTasks[form.ageBand]?.summary || "연령 흐름을 선택해 주세요."}</strong>
    </div>

    <div style={styles.row}>
      <Field label="성장과제">
        <div style={styles.chips}>
          {getGrowthTasksForAge(form.ageBand).map((task) => (
            <Chip
              key={task.key}
              active={form.growthTask === task.key}
              onClick={() =>
                setForm((prev) => ({
                  ...prev,
                  growthTask: task.key,
      ...
      <Field label="세부 키워드">
        <div style={styles.chips}>
          {(getSelectedGrowthTask(form)?.keywords || []).map((keyword) => (
            <Chip
              key={keyword}
              active={form.growthTaskKeywords.includes(keyword)}
              onClick={() => toggleArray("growthTaskKeywords", keyword)}
            >
              {keyword}
            </Chip>
          ))}
    ...
    {getSelectedGrowthTask(form) && (
      <div style={styles.previewBox}>
        <strong>현재 선택된 성장 구조</strong>
        {"\n\n"}
        {buildGrowthTaskSummary(form)}
        {"\n\n"}
        <strong>학부모용 쉬운 해석</strong>
        {"\n"}
  ...
  <div style={styles.softBox}>
    <h3>사회성코칭</h3>
    <div style={styles.sectionHint}>
      집과 학교에서는 놓치기 쉬운 또래 관계 장면을 수업 안에서 관찰하고 코칭한 방향입니다.
    </div>
  ...
  <div style={{ ...styles.softBox, marginTop: 18 }}>
    <h3>이번달 성장 제목</h3>
    <div style={styles.sectionHint}>
      수업 장면과 성장 의미를 한 줄로 압축한 심화브리핑 제목입니다. 추천 문구를 선택하거나 직접 수정할 수 있습니다.
    </div>

    <div style={{ marginTop: 12 }}>
      <Field label="제목 직접 수정">
        <input
          style={styles.input}
          value={form.briefingTitle || ""}
          onChange={(event) => update("briefingTitle", event.target.value)}
          placeholder={briefingTitleCandidates[0] || "예: 스케치 안에서 자기 생각이 자라난 시간"}
        />
      </Field>
    </div>

    <div style={{ marginTop: 12 }}>
      <div style={styles.label}>자동 추천</div>
      <div style={{ display: "grid", gap: 8 }}>
        {briefingTitleCandidates.map((title) => (
          <button
            key={title}
            type="button"
            style={{
              ...styles.secondaryBtn,
              textAlign: "left",
              borderColor: form.briefingTitle === title ? "#0f172a" : "#cbd5e1",
            }}
            onClick={() => update("briefingTitle", title)}
          >
            {title}
          </button>
        ))}
...
<section style={styles.card}>
  <h2 style={styles.sectionTitle}>자라다 성장 구조</h2>

  <div style={styles.sectionHint}>
    교사가 브리핑을 작성할 때 기준으로 삼는 자라다의 성장 지도입니다.
  </div>

  <div style={{ ...styles.softBox, marginTop: 18 }}>
    <h3>1. 성장의 큰 흐름</h3>

    <div style={{ display: "grid", gap: 4, lineHeight: 1.35, color: "#334155" }}>
      <div>건강한 자립</div>
      ...
      <div>반복된 자아실현</div>
      <div style={{ color: "#94a3b8" }}>↓</div>
      <div>자아효능감 · 사회적효능감</div>
      <div style={{ color: "#94a3b8" }}>↓</div>
      <div>인지적 자기조절 · 정서적 자기조절</div>
      <div style={{ color: "#94a3b8" }}>↓</div>
      <div>소속감 · 효능감 · 안전감</div>
    </div>
  </div>
  ...
  <div style={{ ...styles.softBox, marginTop: 16 }}>
    ...
    <div style={{ display: "grid", gap: 12, lineHeight: 1.7, color: "#334155" }}>
      <div>
        <strong>6–7세</strong>
        <br />
        행동적 자기조절 · 내면적 자기조절
        <br />
        <span style={{ color: "#64748b" }}>교실규칙 / 만족지연 / 교사신뢰 / 분리안정</span>
      </div>
      ...
      <div>
        ...
        인지적 자기조절 · 사회적 자기조절
        <br />
        <span style={{ color: "#64748b" }}>과제설정 / 계획 / 완성 / 의견조율 / 인정받기 / 도구숙달</span>
      </div>

      <div>
        <strong>12–13세</strong>
        <br />
        메타인지적 자기조절 · 정서적 자기경영
        <br />
        <span style={{ color: "#64748b" }}>자신의 강점·약점 이해 / 한계넘기 / 내적동기 / 역할과 정체성</span>
      </div>
    </div>
  </div>
  ...
  <div style={{ ...styles.softBox, marginTop: 16 }}>
    ...
    <div style={{ display: "grid", gap: 10, lineHeight: 1.7, color: "#334155" }}>
      <div>
        <strong>인지적 자기조절</strong>
        <br />
        6–7세: 행동적 → 8–11세: 인지적 → 12세 이상: 메타인지적
      </div>

      <div>
        <strong>정서적 자기조절</strong>
        <br />
        6–7세: 내면적 → 8–11세: 사회적 → 12세 이상: 정서적 자기경영
      </div>
    </div>
  </div>
</section>

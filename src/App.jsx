...
const keywordPhraseMap = {
  감정인식: ["자신의 감정 변화를 알아차리며 이어가보는 모습", "상황 안에서 감정을 스스로 인식해보는 흐름"],
  감정표현: ["감정을 자연스럽게 표현해보는 모습", "자신의 상태를 바깥으로 드러내보는 경험"],
  감정말하기: ["감정을 말로 꺼내보는 시도", "자신의 마음을 언어로 표현해보는 경험"],
  멈추기: ["바로 반응하기보다 잠시 멈춰보는 모습", "하고 싶은 흐름을 잠깐 조절해보는 경험"],
  기다리기: ["자신의 순서를 기다려보는 경험", "흐름을 급하게 가져가지 않고 기다려보는 모습"],
  차례지키기: ["정해진 순서를 지켜보는 모습", "관계 안에서 자신의 차례를 받아들이는 경험"],
  목표정하기: ["스스로 해야 할 방향을 정해보는 모습", "자신만의 목표를 세워보는 경험"],
  시작하기: ["망설임보다 먼저 시도해보는 모습", "생각에 머물지 않고 바로 시작해보는 경험"],
  선택하기: ["여러 가능성 중 하나를 골라보는 경험", "자신의 방식으로 선택해보는 모습"],
  단계나누기: ["과정을 나누어 생각해보는 힘", "한 번에 하기보다 단계를 나눠 접근해보는 모습"],
  단계수행: ["정해진 순서대로 이어가보는 힘", "한 단계씩 작업을 이어가는 모습"],
  과제지속: ["쉽지 않은 흐름도 계속 이어가보는 힘", "중간에 멈추지 않고 붙잡아보는 모습"],
  완성경험: ["결과를 완성해보며 성취를 느끼는 경험", "끝까지 이어가며 해냈다는 감각을 쌓는 모습"],
  의견조율: ["서로 다른 생각을 맞춰보는 경험", "관계 안에서 의견을 조정해보는 모습"],
  갈등해결: ["문제 상황을 풀어보는 경험", "부딪히는 상황을 관계 안에서 정리해보는 모습"],
  역할수행: ["맡은 역할을 끝까지 해보는 경험", "자신의 역할을 책임 있게 이어가는 모습"],
  주제지속: ["한 가지 주제를 계속 붙잡아보는 경험", "주제를 이어가며 깊이를 더해보는 모습"],
  반복탐구: ["반복 속에서 표현을 발전시켜보는 경험", "같은 시도를 이어가며 차이를 만들어보는 모습"],
  관찰심화: ["관찰을 깊게 이어가며 표현을 다듬어보는 흐름", "하나의 대상을 더 세밀하게 살펴보는 경험"],
  표현확장: ["표현 방식을 넓혀보는 경험", "기존 방식에서 한 걸음 더 확장해보는 모습"],
  몰입경험: ["작업에 깊게 집중해보는 경험", "한 가지 흐름에 오래 머물러보는 모습"],
  발전흐름: ["반복 속에서 발전을 확인하는 경험", "시도와 보완이 쌓이며 변화가 드러나는 흐름"],
  협력경험: ["함께 맞춰가며 작업해보는 경험", "다른 친구와 협력해보는 모습"],
  의사소통: ["의견을 주고받으며 이어가보는 경험", "말과 반응을 통해 흐름을 맞춰보는 모습"],
  관계조율: ["관계 안에서 흐름을 맞춰가는 경험", "서로 다른 입장을 조정하며 이어가보는 모습"],
  공동성취: ["함께 결과를 만들어가는 경험", "공동의 목표를 완성해보는 흐름"],
  도전극복: ["어려운 과정도 버티며 넘어서는 모습", "쉽지 않은 시도를 끝까지 이어가보는 경험"],
  완수경험: ["끝까지 완수해보며 성취를 쌓는 경험", "큰 작업을 해냈다는 감각으로 이어지는 흐름"],
};
...
[브리핑 목표]
아이의 수업 장면과 작품의 변화 흐름을 바탕으로,
성장의 과정을 해석하고 학부모가 이해할 수 있는 언어로 전달한다.

[작성 원칙]
- 결과보다 과정과 변화 흐름을 중심으로 작성한다.
- 관찰 메모와 작품 흐름을 하나의 이야기로 연결한다.
- 키워드는 직접 쓰지 말고 의미만 자연스럽게 녹인다.
- 연령, 재원기간, 발달단계 같은 표현은 직접 쓰지 않는다.
- '메모는 짧지만', '간단히 보면', '요약하면' 같은 메타 표현은 절대 사용하지 않는다.
- 평가하지 말고 관찰 기반으로 신뢰감 있게 작성한다.
- 문장은 자연스럽고 부드럽게, 학부모가 이해하기 쉽게 작성한다.
- 같은 문장 구조 반복 금지
- 키워드 나열 금지
- 반드시 “흐름 → 연결 → 해석” 구조로 작성
...
- 비전 브리핑 문장: ${(visionResult?.briefing_lines || []).join(" / ")}
...
[최종 지시]
위 기준을 모두 반영하여
학부모에게 바로 전달 가능한 자라다식 브리핑을 작성하라.`;
}
...
export default function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("jarada-briefing-records-v5");
    if (saved) setRecords(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem("jarada-briefing-records-v5", JSON.stringify(records));
  }, [records]);

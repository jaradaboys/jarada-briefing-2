...
alert(`${incoming.length}개의 JARVIS 관찰일지를 불러왔고, 기본값을 업데이트했습니다.`);
 ...
 <div style={styles.sectionHint}>
  ...
  <strong>API 연계 준비 중 · 현재는 임시 확인값 사용</strong>
                {currentStudentProfile?.googleFormSource && (
  <div style={{ fontSize: 13, color: "#475569", marginTop: 8 }}>
    연결된 구글폼 응답: {currentStudentProfile.googleFormSource.submittedAt} / 연락처 뒷자리{" "}
    {currentStudentProfile.googleFormSource.phoneLast4}
  </div>
)}
</div>
<div style={{ marginTop: 12 }}>
  <button style={styles.secondaryBtn} onClick={fetchGoogleFormNeeds}>
    구글폼 응답 불러오기
  </button>

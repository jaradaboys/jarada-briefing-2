...
{(form.parentNeeds.homeDirection ||
  ...
  <div style={styles.previewBox}>
    ...
    {"\n"}
    - 누적 관찰 수: {form.growthProfile.sourceCount}개
    {"\n"}
    - 기준일: {form.growthProfile.updatedUntil || "없음"}
    {"\n"}
    - 전체 핵심 흐름:{" "}
    {form.growthProfile.dominantMainKeywords.join(" → ") || "아직 분석된 흐름이 없습니다."}
    {"\n"}
    - 현재 성장 방향: {form.growthProfile.longTermFlow || "집중브리핑 누적 후 자동 생성됩니다."}

    {"\n\n"}
    <strong>2. 연차별 성장 흐름</strong>
    {"\n"}
    - 1년차: 적응, 라포 형성, 애정 주제 기반 자기표현의 시작
    {"\n"}
    - 2년차: 반복 작업을 통한 몰입 유지와 과제 지속 경험 확장
    {"\n"}
    - 3년차: 관계 속 표현, 감정 조절, 사회성 코칭 흐름 강화
    {"\n"}
    - 이후 흐름: 누적 집중브리핑을 기준으로 자동 정교화 예정

    {"\n\n"}
    <strong>3. 올해 분기별 성장 흐름</strong>
    {"\n"}
    - 1분기: 새로운 수업 방향 적응, 사회성 수업 이해, 표현 방식 조정
    {"\n"}
    - 2분기: 관계 조율, 긍정 언어, 도움 요청과 거절 수용 연습
    {"\n"}
    - 3분기: 애정 주제 확장, 자기표현과 몰입 경험 강화
    {"\n"}
    - 4분기: 해야 하는 것과 하고 싶은 것의 균형, 계획과 마무리 경험

    {"\n\n"}
    <strong>4. 지난달까지의 기본값</strong>
    {"\n"}
    - 현재 아이의 중심 성장축:{" "}
    {form.growthProfile.dominantMainKeywords.join(", ") || "아직 없음"}
    {"\n"}
    - 반복 강점: {form.growthProfile.repeatedStrengths || "아직 없음"}
    {"\n"}
    - 반복 어려움: {form.growthProfile.repeatedNeeds || "아직 없음"}
    {"\n"}
    - 최근 변화: {form.growthProfile.recentTrend || "아직 없음"}
    {"\n"}
    - 이번달 브리핑에서 봐야 할 관찰 포인트:{" "}
    {form.growthProfile.nextFocus || "이번 달 수업 안에서 새롭게 보인 변화와 반복되는 어려움을 함께 관찰합니다."}
  </div>
)}
